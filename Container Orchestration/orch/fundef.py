...
def stop_relaunch(port):
	...
	os.system(command)
	command='sudo '+'docker '+'run '+'-d '+'-v '+' /home/ubuntu/data:/data '+'-p '+str(port)+':'+str(80)+' acts:latest'
	os.system(command)
	return
"""
def stop_write(port):
...
def launch():
	...
	l.append(tp)
	a=int(int(a)+1)
	command='sudo '+'docker '+'run '+'-d '+' -v '+'/home/ubuntu/data:/data'+' -p '+str(a)+':'+str(80)+' acts:latest'
